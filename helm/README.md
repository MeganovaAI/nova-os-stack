...
A partner tenant's end users should see the partner's brand in the URL bar, not
the infra's. Set `ingress.host` to the partner's host (it overrides
`ingress.domain`), have the partner CNAME it at the ingress LB, and let
