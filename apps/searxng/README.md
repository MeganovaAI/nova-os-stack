...
Self-hosted meta-search aggregator. Provides Libra OS's `web` skill with snippet-level search results without a per-query API cost.

Image: [`searxng/searxng`](https://github.com/searxng/searxng) — **AGPL-3.0**.

## License posture

Libra OS communicates with SearXNG over its public HTTP API only and does not bundle, link against, or ship SearXNG. Customers running this stack are responsible for complying with SearXNG's AGPL terms (which is straightforward for self-hosted use). If your legal team needs a non-AGPL alternative, swap in the `Fetcher` interface — see Libra OS docs.
