...
Libra OS runs HTTP-only internally; the partner's existing infrastructure
...
- Partner runs Libra OS on a host without public exposure (private subnet
...
1. **Pass `Host` header verbatim** — Libra OS uses the request's Host to
   build OIDC redirect URIs. If the proxy rewrites Host, OIDC breaks.
2. **Forward `X-Forwarded-Proto: https`** — Libra OS reads this to decide
